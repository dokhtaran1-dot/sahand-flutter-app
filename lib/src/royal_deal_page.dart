...
class _RoyalDealPageState extends State<RoyalDealPage> {
  ...
  Future<void> _startMusic() async {
    ...
    try {
      ...
      await _music.play(AssetSource('audio/royal_deal_theme.mp3'));
    } catch (_) {}
  }
  ...
  Future<void> _fx(String name, {double volume = .8}) async {
    ...
    try {
      ...
      await _sfx.play(AssetSource('audio/$name'));
    } catch (_) {}
  }
  ...
  void _pick(int i) {
    ...
    _fx(value >= 3000 ? 'case_big.mp3' : 'case_open.mp3');
    ScaffoldMessenger.of(context)
      ..hideCurrentSnackBar()
      ..showSnackBar(SnackBar(backgroundColor: const Color(0xFF18090B), content: Text('جعبه ${i + 1}:  $value تیکت R1', textAlign: TextAlign.center, textDirection: TextDirection.rtl)));
  ...
  void _callBanker() {
    ...
    _fx('banker_ring.mp3', volume: 1);
  }

  void _deal() {
    final win = bankerOffer ?? 0;
    setState(() => ended = true);
    _fx('deal_win.mp3', volume: 1);
    _result('DEAL 🤝', 'پیشنهاد بانکدار را قبول کردی\n$win تیکت R1');
  }

  void _noDeal() {
    _fx('no_deal.mp3', volume: .95);
    setState(() { bankerOffer = null; round++; openedThisRound = 0; });
    ...
    if (unopenedOthers.isEmpty) {
      ...
      setState(() => ended = true);
      _fx(win >= 3000 ? 'jackpot.mp3' : 'final_reveal.mp3', volume: 1);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.black, foregroundColor: gold, centerTitle: true,
        title: const Text('👑 ROYAL DEAL'),
        actions: [IconButton(onPressed: _toggleSound, tooltip: soundOn ? 'قطع صدا' : 'پخش صدا', icon: Icon(soundOn ? Icons.volume_up_rounded : Icons.volume_off_rounded))],
      ),
          ...
          Expanded(child: GridView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              return InkWell(onTap: () => _pick(i), borderRadius: BorderRadius.circular(14), child: AnimatedContainer(
                ...
                decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), border: Border.all(color: isMine ? Colors.redAccent : gold, width: isMine ? 2.2 : 1.2), gradient: LinearGradient(colors: isOpen ? [const Color(0xFF3B0A10), const Color(0xFF170507)] : [const Color(0xFF2B2415), const Color(0xFF090909)]), boxShadow: [BoxShadow(color: (isMine ? Colors.red : gold).withOpacity(.24), blurRadius: 8)]),
                child: Center(child: isOpen ? Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.confirmation_number_outlined, color: gold), const SizedBox(height: 4), Text('${_caseValues[i]}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), const Text('R1', style: TextStyle(color: gold, fontSize: 11))]) : Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.business_center, color: gold, size: 30), const SizedBox(height: 4), Text('${i + 1}', style: const TextStyle(color: Colors.white, fontSize: 21, fontWeight: FontWeight.bold)), if (isMine) const Text('جعبه من', textDirection: TextDirection.rtl, style: TextStyle(color: Colors.redAccent, fontSize: 10))])),
              ));
            },
          )),
          if (_myCase != null) Padding(padding: const EdgeInsets.fromLTRB(18, 8, 18, 16), child: Text('جایزه‌ها: 10 تا 10,000 تیکت R1 • خرید • رستوران • VIP • پارکینگ • رویدادها', textAlign: TextAlign.center, textDirection: TextDirection.rtl, style: TextStyle(color: Colors.white.withOpacity(.75), fontSize: 12))),
        ]),
        if (bankerOffer != null) Positioned.fill(child: Container(color: Colors.black.withOpacity(.88), child: Center(child: Container(
          margin: const EdgeInsets.all(24), padding: const EdgeInsets.all(24), decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), border: Border.all(color: gold, width: 1.5), gradient: const LinearGradient(colors: [Color(0xFF2D0710), Color(0xFF090909)])),
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Icon(Icons.phone_in_talk, color: gold, size: 46), const SizedBox(height: 10),
            ...
            const SizedBox(height: 18), const Text('پیشنهاد فعلی', textDirection: TextDirection.rtl, style: TextStyle(color: gold)),
            Text('${bankerOffer!}', style: const TextStyle(color: Colors.white, fontSize: 42, fontWeight: FontWeight.w900)), const Text('R1 TICKETS', style: TextStyle(color: gold, letterSpacing: 2)), const SizedBox(height: 22),
            Row(children: [Expanded(child: ElevatedButton(onPressed: _deal, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1B7A43), minimumSize: const Size.fromHeight(52)), child: const Text('DEAL\nقبول می‌کنم', textAlign: TextAlign.center))), const SizedBox(width: 12), Expanded(child: ElevatedButton(onPressed: _noDeal, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF9E0B19), minimumSize: const Size.fromHeight(52)), child: const Text('NO DEAL\nادامه می‌دهم', textAlign: TextAlign.center)))])
          ]),
        )))),
      ])),
    );
